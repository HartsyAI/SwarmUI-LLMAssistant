...
> | 10 | Implemented: pins core `WildcardSeed` to a deterministic FNV-1a hash of the source prompt (user-set seed wins). Real-generation testing then exposed that the WHOLE `<llmprompt>` handler never fired — core's `PreparsePromptLikes` is element 0 of `LateSpecialParameterHandlers` and consumed the tags before our `Add()`ed handler ran; fixed with `Insert(0)` (branch `fix/llmprompt-ordering`) | **Fully live-verified** with Z-Image Turbo on the 3060 + local StableLM GGUF: tag expands to real LLM output, `llm*` params consumed, wildcard-seed pinned identically across a batch (both images picked "golden"), no-param control diverged (crimson/violet per image seed) |
